...
export default function AppHomePage() {
  ...
  async function handleLogout() {
    setLoggingOut(true)
    try { await logout() } catch { /* clear token regardless */ }
    clearToken()
    router.push('/auth')
  }
  ...
  function handleArchived(projectId: string) {
    setProjects(prev =>
      prev.map(p => p.id === projectId ? { ...p, archived_at: new Date().toISOString() } : p)
    )
  }
  ...
  const activeShared    = projects.filter(p => !p.is_owner && p.share_status === 'active')
  const archivedOwn     = projects.filter(p => p.is_owner && !!p.archived_at)
  const revokedShared   = projects.filter(p => !p.is_owner && p.share_status === 'revoked')
