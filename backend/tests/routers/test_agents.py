...
async def test_update_template_agent_forbidden(client):
    ...
    template_agent = next(a for a in detail["agents"] if a["is_template_agent"])
    resp = await client.patch(
...
async def test_delete_custom_agent(client):
    ...
    # Confirm agent no longer appears in project detail
    agents = (await client.get(f"/projects/{proj['id']}")).json()["agents"]
