    ...
    <div className={`bg-neutral-900 border ${agentBorderClass(agent.status)} rounded-xl p-5 flex flex-col gap-4`}>
      ...
      {agent.is_template_agent ? (
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-neutral-800 w-fit">
          ...
          <span className="text-xs text-neutral-500">Default team · edit from <Link href="/agents" className="text-primary-400 hover:text-primary-300">global settings</Link></span>
        </div>
      ) : editing ? (
...
// Inline add-agent form inside the page
...
export default function AgentsPage() {
  ...
  if (fetchError) return (
    <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center gap-4">
      ...
      <Link href={`/projects/${projectId}`} className="text-sm text-primary-400 hover:text-primary-300">← Back to Dashboard</Link>
    </div>
  )
    ...
    <div className="min-h-screen bg-neutral-950">
      <header className="border-b border-neutral-800 bg-neutral-900">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center gap-4">
          <Link href={`/projects/${projectId}`} className="text-sm text-neutral-400 hover:text-neutral-100 transition-colors flex items-center gap-1">
          ...
          <Link href="/agents" className="text-xs text-primary-400 hover:text-primary-300 transition-colors hidden sm:block">
            Edit default team →
          </Link>
        </div>
      </header>
